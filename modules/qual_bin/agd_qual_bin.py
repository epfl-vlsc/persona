...
class QualBinService(Service):
    ...
    def add_graph_args(self, parser):
        parser.add_argument("-p", "--parse-parallel", default=5, type=numeric_min_checker(minimum=1, message="read parallelism"),
                            help="total paralellism level for reading data from disk")
        parser.add_argument("-w", "--write-parallel", default=15, help="number of writers to use",
                            type=numeric_min_checker(minimum=1, message="number of writers min"))
        parser.add_argument("-pp", "--process-parallel", default=5, help="adjusting parallelism of quality binning stage",type=numeric_min_checker(minimum=1, message="number of parallel processes"))
        ...
        parser.add_argument("-c", "--parallel-compress", default =30, help="number of compressors", type = numeric_min_checker(minimum=1, message = "number of compressors min"))	
    ...
    def make_graph(self, in_queue, args):
        ...
        op = agd_qual_bin_local(in_queue=in_queue,
                                       ...
                                       parallel_write=args.write_parallel, parallel_process = args.process_parallel, parallel_compress = args.parallel_compress)
        run_once = []
        return [op], run_once 
...
def agd_qual_bin_local(in_queue, outdir=None, parallel_parse=5, parallel_write=15, parallel_compress=30, parallel_process = 5):
    """
    ...
    parsed_result = pipeline.join(parsed_results_list, parallel=parallel_process, capacity=8, multi=True, name = "name_for_debugging")
    ...
    print("output_list")
    print(output_list)
    result_to_write = pipeline.join(output_list, parallel=parallel_compress, 
